...
public class XMozillaStatus {
  ...
  /**
   * Already gone (when folder not compacted). Since actually removing a
   * message from a folder is a semi-expensive operation, we tend to delay it;
   * messages with this bit set will be removed the next time folder compaction
   * is done. Once this bit is set, it never gets un-set.
   */
  private static final int MSG_FLAG_EXPUNGED = 0x0008;
  /**
   * Whether subject has “Re:” on the front. The folder summary uniquifies all
   * of the strings in it, and to help this, any string which begins with “Re:”
   * has that stripped first. This bit is then set, so that when presenting the
   * message, we know to put it back (since the “Re:” is not itself stored in
   * the file).
   */
  ...
  /**
   *  If set, then this message's sender has been authenticated when sending
   *  this msg. This means the POP3 server gave a positive answer to the
   *  XSENDER command. Since this command is no standard and only known by few
   *  servers, this flag is unmeaning in most cases.
   */
  private static final int MSG_FLAG_SENDER_AUTHED = 0x0200;
  /**
   * If set, then this message's body contains not the whole message, and a
   * link is available in the message to download the rest of it from the POP
   * server. This can be only a few lines of the message (in case of size
   * restriction for the download of messages) or nothing at all (in case of
   * “Fetch headers only”)
   */
  private static final int MSG_FLAG_PARTIAL = 0x0400;
  /**
   * If set, this message is queued for delivery. This only ever gets set on
   * messages in the queue folder, but is used to protect against the case of
   * other messages having made their way in there somehow – if some other
   * program put a message in the queue, it won't be delivered later!
   */
  ...
  /**
   * If set, this message is marked as deleted on the server. This only applies
   * to messages on IMAP servers.
   */
  private static final int MSG_FLAG_IMAP_DELETED = 0x00200000;
  /**
   * This message required to send a MDN (Message Disposition Notification) to
   * the sender of the message. For information about MDN see Wikipedia:Return
   * receipt.
   */
  private static final int MSG_FLAG_MDN_REPORT_NEEDED = 0x00400000;
  /**
   * An MDN report message has been sent for this message. No more MDN report
